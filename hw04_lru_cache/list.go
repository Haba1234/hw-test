...
func (l *list) PushFront(v interface{}) *ListItem {
	// первая запись? создать первый элемент списка.
	if l.Front() == nil {
	...
	l.elements = append(newSlice, l.elements...)                    // перенесли в новый слайс все элементы старого.
...
func (l *list) Remove(i *ListItem) {
	...
	for num, val := range l.elements {
		if val.Value == i.Value {
			...
			// обновление ссылок у предыдущего и следующего элементов списка.
			if num > 0 {
